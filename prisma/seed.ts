...
async function main() {
  ...
  const propPass = await bcrypt.hash("propietario123", 10);
  const vendPass = await bcrypt.hash("vendedor123", 10);

  await prisma.usuario.upsert({
    where: { email: "admin@botellapp.cl" },
    update: {},
    create: {
      nombre: "Administrador",
      email: "admin@botellapp.cl",
      password: adminPass,
      rol: "ADMIN",
    },
  });
  ...
  await prisma.usuario.upsert({
    where: { email: "propietario@botellapp.cl" },
    update: {},
    create: {
      nombre: "Carlos Rodríguez",
      email: "propietario@botellapp.cl",
      password: propPass,
      rol: "PROPIETARIO",
    },
  });
  ...
  await prisma.usuario.upsert({
    where: { email: "vendedor@botellapp.cl" },
    update: {},
    create: {
      nombre: "Ana González",
      email: "vendedor@botellapp.cl",
      password: vendPass,
      rol: "VENDEDOR",
    },
  });
  ...
  // Productos
  const categoriaIds = await prisma.categoria.findMany({ select: { id: true, nombre: true } });
  const getCatId = (nombre: string) => categoriaIds.find(c => c.nombre === nombre)?.id || 1;
  ...
  await prisma.configuracionTienda.upsert({
    ...
    update: {},
    create: {
      nombre: "BOTELLAPP Licorería",
      rut: "12.345.678-9",
      direccion: "Av. Las Licorerías 123, Santiago",
      telefono: "+56 9 8765 4321",
      email: "info@botellapp.cl",
